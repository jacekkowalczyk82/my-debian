...
lb config --debian-installer live -d buster \
 ...
 --iso-application debian-live-10-ldd-kde \
 --iso-volume debian-live-10-ldd-kde \
...
#build ISO
lb build --debug --verbose 2>&1 |tee debian-live-10-ldd-kde-`date '+%Y-%m-%d_%H%M%S'`.log

#`date '+%Y-%m-%d_%H%M%S'`
GENERATED_NAME="debian-live-10-ldd-kde-amd64"
RELEASE_NAME="debian-live-10-ldd-kde-`date '+%Y-%m-%d_%H%M%S'`-amd64"
...
if [[ -e "${GENERATED_NAME}.hybrid.iso" ]]; then 
    ...
    FILES_PATTERN="debian-live-10-ldd-kde"
